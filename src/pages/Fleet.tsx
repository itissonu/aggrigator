...
// Mock data matching the columns needed
const fleetData = [
  { sno: 1, regNo: 'DL1RTB6821', vClass: 'Passenger', vType: 'Autorickshaw', cat: '4W', fuel: 'CNG', rc: 'Expiring on 01 Nov, 2031', insurance: 'ACTIVE', pucc: 'Active', license: 'Verified' },
  { sno: 2, regNo: 'DL1ZC6799', vClass: 'Passenger', vType: 'Motor Cab', cat: '4W', fuel: 'CNG', rc: 'Expiring on 22 May, 2034', insurance: 'ACTIVE', pucc: 'Expiring Soon', license: 'Verified' },
  { sno: 3, regNo: 'DL1ZD2122', vClass: 'Passenger', vType: 'Motor Cab', cat: '4W', fuel: 'CNG', rc: 'Expiring on 25 Mar, 2036', insurance: 'Expiring Soon', pucc: 'Active', license: 'Pending' },
  { sno: 4, regNo: 'DL1RTA8539', vClass: 'Passenger', vType: 'Maxicab', cat: '4W', fuel: 'CNG', rc: 'Expiring on 01 Jun, 2031', insurance: 'ACTIVE', pucc: 'Active', license: 'Verified' },
  { sno: 5, regNo: 'DL1ZB1277', vClass: 'Passenger', vType: 'Autorickshaw', cat: '4W', fuel: 'CNG', rc: 'Expiring on 16 Mar, 2032', insurance: 'Expiring Soon', pucc: 'Active', license: 'Verified' },
  { sno: 6, regNo: 'DL52GD0957', vClass: 'Passenger', vType: 'Motor Cab', cat: '4W', fuel: 'Electric', rc: 'Expiring on 21 Apr, 2037', insurance: 'ACTIVE', pucc: 'Not Applicable', license: 'Verified' },
];
        ...
        <div className="glass rounded-xl border border-border p-6 shadow-sm bg-background">
          ...
          <div className="overflow-x-auto">
            <table className="w-full text-sm">
              ...
              <tbody>
                {fleetData.map((row, i) => (
                  <tr key={i} className="border-b border-border/40 hover:bg-muted/10 transition-colors last:border-0">
                    <td className="py-4 px-2 font-medium text-muted-foreground whitespace-nowrap">{row.sno}</td>
                    <td className="py-4 px-2 font-mono font-medium whitespace-nowrap">{row.regNo}</td>
                    <td className="py-4 px-2 text-muted-foreground whitespace-nowrap">{row.vClass}</td>
                    <td className="py-4 px-2 font-medium text-foreground whitespace-nowrap">{row.vType}</td>
                    <td className="py-4 px-2 text-muted-foreground whitespace-nowrap">{row.fuel}</td>
                    <td className="py-4 px-2 text-muted-foreground whitespace-nowrap">{row.rc}</td>
                    <td className="py-4 px-2 whitespace-nowrap">
                      {row.insurance === 'ACTIVE' ? (
                        <span className="text-foreground font-medium uppercase">{row.insurance}</span>
                      ) : (
                        <span className="bg-red-500 text-white px-2.5 py-1 rounded-md text-xs font-semibold shadow-sm">{row.insurance}</span>
                      )}
                    </td>
                    <td className="py-4 px-2 whitespace-nowrap">
                      {row.pucc === 'Active' || row.pucc === 'Not Applicable' ? (
                        <span className="text-muted-foreground">{row.pucc}</span>
                      ) : (
                        <span className="bg-red-500 text-white px-2.5 py-1 rounded-md text-xs font-semibold shadow-sm">{row.pucc}</span>
                      )}
                    </td>
                    <td className="py-4 px-2 whitespace-nowrap">
                      <span className={`px-2 py-1 rounded text-xs font-bold ${row.license === 'Verified' ? 'bg-green-100 text-green-700' : 'bg-amber-100 text-amber-700'}`}>
                        {row.license}
                      </span>
                    </td>

...
export default function Login() {
  ...
  const handleVerify = () => {
    if (otp.length >= 4) {
      login(activeTab);
      navigate(activeTab === 'officer' ? '/admin' : '/dashboard');
    }
  };
        ...
        <div className="w-full max-w-md">
          ...
          <div className="flex gap-1 p-1 bg-secondary rounded-lg mb-8">
            {tabs.map((t) => (
              <button
                key={t.role}
                onClick={() => { setActiveTab(t.role); setOtpSent(false); setOtp(''); }}
                className={cn(
          ...
          <div className="space-y-4">
            ...
            {!otpSent ? (
              <Button variant="accent" className="w-full gap-2" onClick={handleSendOTP} disabled={phone.length < 10}>

...
import { feeStructure, mockTransactions, formatINR } from '@/lib/mockData';
  ...
  return (
    <DashboardLayout>
      ...
      <div className="grid sm:grid-cols-2 gap-4 mb-8">
        <StatCard label="Wallet Balance" value={formatINR(245000)} icon={<WalletIcon className="w-5 h-5" />} trend="Last topped up 10/03/2026" />
        <StatCard label="Total Fees Due" value={formatINR(18500)} icon={<CreditCard className="w-5 h-5" />} trend="Q1 2026" />
      </div>
          ...
          {mockTransactions.map((t) => (
            <div key={t.id} className="px-5 py-3 flex items-center justify-between">
              <div className="flex items-center gap-3">
                <div className={`p-1.5 rounded-lg ${t.amount > 0 ? 'bg-accent/10 text-accent' : 'bg-destructive/10 text-destructive'}`}>
                  {t.amount > 0 ? <ArrowDownLeft className="w-4 h-4" /> : <ArrowUpRight className="w-4 h-4" />}
                </div>
                <div>
                  <p className="text-xs font-medium">{t.description}</p>
                  <p className="text-[10px] text-muted-foreground">{t.date} · {t.id}</p>
                </div>
              </div>
              <div className="text-right">
                <p className={`text-sm font-semibold font-mono ${t.amount > 0 ? 'text-accent' : 'text-foreground'}`}>

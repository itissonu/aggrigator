...
import { mockCompanies, mockApplications, mockPenalties, mockAdminGrievances, mockFleetVerify, formatINR } from '@/lib/mockData';
...
const chartData = mockCompanies.map(c => ({ name: c.name, ev: c.evPercent, nonEv: 100 - c.evPercent }));

export default function Admin() {
  const totalFleet = mockCompanies.reduce((s, c) => s + c.fleetSize, 0);
  const avgCompliance = Math.round(mockCompanies.reduce((s, c) => s + c.complianceRate, 0) / mockCompanies.length);
  const totalFees = mockCompanies.reduce((s, c) => s + c.walletBalance, 0);
  ...
  return (
    <DashboardLayout>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4 mb-8">
        <StatCard label="Licensed Aggregators" value={mockCompanies.length} icon={<Shield className="w-5 h-5" />} />

            ...
            {step === 4 && (
              <div className="space-y-6">
                <div className="glass rounded-xl border border-border p-8 max-w-lg mx-auto text-center">
                  ...
                  <Button className="w-full" size="lg"><CreditCard className="w-4 h-4 mr-2" />Pay ₹{totalAmount} from Wallet</Button>
          ...
          <div className="space-y-6">
              ...
              <div className="glass rounded-xl border border-border p-6">
                ...
                <div className="space-y-3">
                  <div>
                    ...
                    <input type="text" placeholder="DL-0420110012345" className="w-full px-4 py-2.5 border border-border rounded-lg text-sm bg-background focus:ring-2 focus:ring-primary/30 outline-none" />
                  </div>
                  <div>
                    ...
                    <select className="w-full px-4 py-2.5 border border-border rounded-lg text-sm bg-background">
                      <option>Select validated vehicle…</option>
                      {validVehicles.map(v => <option key={v.reg} value={v.reg}>{v.reg} — {v.model}</option>)}
                    </select>
                  </div>
                  <Button className="w-full">Verify & Link</Button>
                </div>
              </div>
            </div>
            ...
            <div className="glass rounded-xl overflow-hidden border border-border">
              <div className="p-4 border-b border-border bg-muted/50 flex items-center justify-between">
                ...
                <span className="text-xs text-muted-foreground">{driverData.length} drivers linked</span>
              </div>
              <div className="overflow-x-auto">
                <table className="w-full text-sm">
                  <thead><tr className="border-b border-border bg-muted/30 text-left">
                  ...
                  <tbody>
                    {driverData.map(d => (
                      <tr key={d.dl} className="border-b border-border last:border-0 hover:bg-muted/20">
                        <td className="p-3 text-muted-foreground">{d.sno}</td>
                        <td className="p-3 font-medium">{d.name}</td>
                        <td className="p-3 font-mono text-xs">{d.dl}</td>
                        <td className="p-3 font-mono text-xs text-primary">{d.vehicle}</td>
                        <td className="p-3 text-muted-foreground">{d.expiry}</td>
                        <td className="p-3">{d.psv ? <Check className="w-4 h-4 text-green-600" /> : <X className="w-4 h-4 text-red-500" />}</td>
                        <td className="p-3"><span className={`px-2 py-0.5 rounded-full text-[10px] font-bold ${d.status === 'verified' ? 'bg-green-100 text-green-700' : 'bg-amber-100 text-amber-700'}`}>{d.status}</span></td>
                      </tr>
                    ))}
                  </tbody>
                </table>

...
import { getDaysTo2030, formatINR, mockCompanies } from '@/lib/mockData';
import { PieChart, Pie, Cell, ResponsiveContainer } from 'recharts';

const company = mockCompanies[0]; // Ola
const evData = [
  { name: 'EV', value: company.evPercent },
  { name: 'Non-EV', value: 100 - company.evPercent },
];
const COLORS = ['hsl(153,100%,42%)', 'hsl(220,14%,89%)'];

const categories = [
  { label: '2-Wheeler', current: 85, target: 100, status: 'green' },
  { label: '3-Wheeler', current: 18, target: 25, status: 'amber' },
  { label: '4-Wheeler', current: 12, target: 15, status: 'red' },
];
  ...
  return (
    <DashboardLayout>
      <div className="mb-6">
        ...
        <p className="text-sm text-muted-foreground">Welcome back, Rajesh Kumar · Ola</p>
      </div>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4 mb-8">
        ...
        <StatCard label="Fleet Size" value={company.fleetSize.toLocaleString('en-IN')} icon={<Car className="w-5 h-5" />} trend="+245 this month" trendUp />
        <StatCard label="Wallet Balance" value={formatINR(company.walletBalance)} icon={<Wallet className="w-5 h-5" />} />
      ...
      <div className="grid lg:grid-cols-3 gap-6">
        ...
        <div className="glass rounded-xl p-5">
          ...
          <div className="text-center -mt-4">
            <p className="text-3xl font-heading font-bold text-accent">{company.evPercent}%</p>
